...
pub fn makePropertyList(comptime Action: type, alloc: Allocator, style: *const Style) !*PropertyList(Action) {
    const ctx = try alloc.create(PropertyList(Action));
...
pub fn PropertyList(comptime Action: type) type {
    return struct {
        ...
        fn update(ctx: ?*anyopaque, available_size: PixelSize) anyerror!void {
            ...
            // We just let ourselves grow vertically forever
            const available_height = std.math.maxInt(u31);
            ...
            const key_available = PixelSize {
                .width = available_size.width -| self.style.value_width,
                .height = available_height,
            };
            ...
            self.size.height = height - self.style.item_pad;
        }
        ...
        fn setInputState(ctx: ?*anyopaque, widget_bounds: PixelBBox, input_bounds: PixelBBox, input_state: InputState) InputResponse(Action) {
            ...
            while (it.next()) |item| {
                const key_response = item.key.setInputState(item.key_box, item.key_box.calcIntersection(input_bounds), input_state);
                mergeInputResponse(Action, key_response, &ret);

                const value_response = item.value.setInputState(item.value_box, item.value_box.calcIntersection(input_bounds), input_state);
    ...
    return struct {
        ...
        fn next(self: *Self)  ?Output {
            ...
            const key_area = PixelBBox {
                ...
                .right = self.widget_left + split_width,
            };

            const value_area = PixelBBox {
                .top = self.y_offs,
                .bottom = self.y_offs + row_height,
                .left = self.widget_left + split_width,
