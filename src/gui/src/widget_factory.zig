...
pub fn widgetState(comptime Action: type, gui_alloc: gui.GuiAlloc, scratch_alloc: *ScratchAlloc, scratch_gl: *GlAlloc) !*WidgetState(Action) {
    ...
    ret.text_renderer = try sphtext.TextRenderer.init(gui_alloc.gl, font_size);
...
pub fn WidgetFactory(comptime Action: type) type {
    return struct {
        ...
        pub fn makeRunnerOrDeinit(self: *const Self, inner: gui.Widget(Action)) !gui.runner.Runner(Action) {
            ...
            return gui.runner.Runner(Action).init(self.alloc.heap.arena(), root_stack.asWidget());
        }
    };
}
