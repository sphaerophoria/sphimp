...
pub fn init(gl_alloc: *GlAlloc) !GlyphAtlas {
...
pub fn getGlyphLocation(
    ...
    if (!gop.found_existing) {
        gop.value_ptr.* = try self.addCharToAtlas(alloc, temp_alloc, scratch_gl, char, point_size, ttf, distance_field_renderer);
    }
    return gop.value_ptr.*;
}

fn addCharToAtlas(self: *GlyphAtlas, alloc: Allocator, temp_alloc: Allocator, scratch_gl: *GlAlloc, char: u8, point_size: f32, ttf: ttf_mod.Ttf, distance_field_renderer: sphrender.DistanceFieldGenerator) !UVBBox {
