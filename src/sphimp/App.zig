...
// FIXME: Runtime configurable
const max_object_name_len = 100;
const max_fonts = 100;
...
pub fn init(alloc: RenderAlloc, scratch_alloc: *ScratchAlloc, scratch_gl: *GlAlloc, window_width: usize, window_height: usize) !App {
    ...
    var shaders = ShaderStorage(ShaderId){
        .alloc = try alloc.makeSubAlloc("shaders"),
    };

    const mul_fragment_shader_id = try shaders.addShader("mask_mul", Renderer.mul_fragment_shader);

    const brushes = ShaderStorage(BrushId){
        .alloc = try alloc.makeSubAlloc("brushes"),
    };

    const fonts = try FontStorage.init(try alloc.heap.makeSubAlloc("fonts"), max_fonts);
...
pub fn load(self: *App, path: []const u8) !void {
    ...
    var new_shaders = ShaderStorage(ShaderId){
        // FIXME: Dedup sub alloc name
        .alloc = try self.alloc.makeSubAlloc("shaders"),
    };
    ...
    for (parsed.shaders) |saved_shader| {
        _ = try new_shaders.addShader(saved_shader.name, saved_shader.fs_source);
    }

    var new_brushes = ShaderStorage(BrushId){
        // FIXME: Dedup sub alloc name
        .alloc = try self.alloc.makeSubAlloc("brushes"),
    };
    // Note that shaders gets swapped in and is freed by this defer
    defer new_brushes.alloc.deinit();

    for (parsed.brushes) |saved_brush| {
        _ = try new_brushes.addShader(saved_brush.name, saved_brush.fs_source);
    }

    var new_fonts = try FontStorage.init(try self.alloc.heap.makeSubAlloc("fonts"), max_fonts);
    // Note that shaders gets swapped in and is freed by this defer
...
pub fn createPath(self: *App) !ObjectId {
    ...
    {
        const obj_alloc = try self.objects.alloc.makeSubAlloc("path");
        errdefer obj_alloc.deinit();

        var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
        name.setContentsTrunc("new path");
    ...
    const mask_id = self.objects.nextId();
    {
        ...
        const obj_alloc = try self.objects.alloc.makeSubAlloc("mask");
        errdefer obj_alloc.deinit();

        var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
        name.setContentsTrunc("new mask");
    ...
    {
        const obj_alloc = try self.objects.alloc.makeSubAlloc("shader_obj");
        errdefer obj_alloc.deinit();

        var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
        name.setContentsTrunc("masked obj");
...
pub fn updateSelectedObjectName(self: *App, name: []const u8) void {
    ...
    selected_object.updateName(name);
}
...
pub fn updateFontId(self: *App, id: FontStorage.FontId) !void {
    const selected_obj = self.selectedObject();
    const text_obj = selected_obj.asText() orelse return error.NotText;
    try text_obj.updateFont(selected_obj.alloc.heap.general(), self.scratch.heap, self.scratch.gl, id, self.fonts, self.renderer.distance_field_generator);
}
...
pub fn updateFontSize(self: *App, requested_size: f32) !void {
    ...
    const selected_obj = self.selectedObject();
    const text_obj = selected_obj.asText() orelse return error.NotText;
    try text_obj.updateFontSize(selected_obj.alloc.heap.general(), self.scratch.heap, self.scratch.gl, size, self.fonts, self.renderer.distance_field_generator);
}
...
pub fn addComposition(self: *App) !ObjectId {
    const obj_alloc = try self.objects.alloc.makeSubAlloc("compositon");
    errdefer obj_alloc.deinit();

    var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
    name.setContentsTrunc("composition");
...
pub fn addDrawing(self: *App) !ObjectId {
    ...
    const obj_alloc = try self.objects.alloc.makeSubAlloc("drawing");
    errdefer obj_alloc.deinit();

    var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
    name.setContentsTrunc("drawing");
...
pub fn addText(self: *App) !ObjectId {
    ...
    const obj_alloc = try self.objects.alloc.makeSubAlloc("text_obj");
    errdefer obj_alloc.deinit();

    var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
    name.setContentsTrunc("text");
    ...
    try self.objects.append(.{
        ...
        .name = name,
        .data = .{
            .text = try obj_mod.TextObject.init(obj_alloc.gl, font_id),
        },
    });
...
pub fn loadImage(self: *App, path: [:0]const u8) !ObjectId {
    const obj_alloc = try self.objects.alloc.makeSubAlloc("fs_image");
    errdefer obj_alloc.deinit();

    const alloc = obj_alloc.heap.general();

    const fs = try obj_mod.FilesystemObject.load(alloc, obj_alloc.gl, path);

    var name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
    name.setContentsTrunc(path);
...
pub fn loadBrush(self: *App, path: [:0]const u8) !BrushId {
    ...
    return self.brushes.addShader(path, fragment_source);
}
...
fn loadFontIntoStorage(scratch_alloc: *ScratchAlloc, path: []const u8, fonts: *FontStorage) !FontStorage.FontId {
    // FIXME: If loading fails for any reason we leak here. Need a way to
    // checkpoint and restore
    const alloc = fonts.alloc.arena();
...
pub fn addShaderFromFragmentSource(self: *App, name: []const u8, fs_source: [:0]const u8) !ShaderId {
    return try self.shaders.addShader(name, fs_source);
}

pub fn addBrushFromFragmnetSource(self: *App, name: []const u8, fs_source: [:0]const u8) !BrushId {
    return try self.brushes.addShader(name, fs_source);
}
...
pub fn addShaderObject(self: *App, name: []const u8, shader_id: ShaderId) !ObjectId {
    ...
    var obj_name = try RuntimeBoundedArray(u8).init(obj_alloc.heap.arena(), max_object_name_len);
    obj_name.setContentsTrunc(name);
    ...
    switch (action orelse return) {
        ...
        .add_draw_stroke => |pos| {
            ...
            if (selected_object.asDrawing()) |d| {
                try d.addStroke(selected_object.alloc.heap.general(), self.scratch.gl, pos, &self.objects, self.renderer.distance_field_generator);
            }
        },
        .add_stroke_sample => |pos| {
            const selected_object = self.selectedObject();
            if (selected_object.asDrawing()) |d| {
                try d.addSample(selected_object.alloc.heap.general(), self.scratch.gl, pos, &self.objects, self.renderer.distance_field_generator);
            }
        },
        .save => {
...
fn regenerateDistanceFields(self: *App) !void {
    ...
    while (obj_it.next()) |obj_id| {
        ...
        try drawing_obj.generateDistanceField(self.scratch.heap.allocator(), self.scratch.gl, &self.objects, self.renderer.distance_field_generator);
    }
}
...
fn regenerateAllTextObjects(self: *App) !void {
    ...
    while (obj_it.next()) |obj_id| {
        ...
        try text_obj.regenerate(obj.alloc.heap.general(), self.scratch.heap, self.scratch.gl, self.fonts, self.renderer.distance_field_generator);
    }
}
