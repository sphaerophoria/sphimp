...
const InputState = struct {
    ...
    fn findCompositionIdx(self: InputState, objects: *Objects, frame_renderer: *Renderer.FrameRenderer) !?obj_mod.CompositionIdx {
        // FIXME: Use hit test object here

        const obj = objects.get(self.selected_object);

        const tex = try frame_renderer.renderCompositionIdMask(obj.*);
        defer tex.deinit();

        const obj_dims = obj.dims(objects);
        // FIXME: Surely there's a fn for this
        // -1, 1 -> 0,obj_dims[0]
        const x_px = std.math.lerp(@as(f32, 0), @as(f32, @floatFromInt(obj_dims[0])), (self.mouse_pos[0] + 1) / 2);
        const y_px = std.math.lerp(@as(f32, 0), @as(f32, @floatFromInt(obj_dims[1])), (self.mouse_pos[1] + 1) / 2);
        const composition_idx = try tex.sample(u32, @intFromFloat(x_px), @intFromFloat(y_px));
