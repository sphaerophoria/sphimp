...
pub const FrameRenderer = struct {
    ...
    pub fn renderCompositionIdMask(self: *FrameRenderer, composition_obj: Object) !Texture {
        ...
        const output_tex = sphrender.makeTextureOfSize(@intCast(composition_object_dims[0]), @intCast(composition_object_dims[1]), .ri32);
        errdefer output_tex.deinit();

        // FIXME: This is definitely duplicated with other stuff
        ...
        temp_viewport.setViewport(@intCast(composition_object_dims[0]), @intCast(composition_object_dims[1]));
        temp_scissor.setAbsolute(0, 0, @intCast(composition_object_dims[0]), @intCast(composition_object_dims[1]));

        var clear_color: [4]i32 = .{ @bitCast(@as(u32, std.math.maxInt(u32))), 0, 0, 0 };
        gl.glClearBufferiv(gl.GL_COLOR, 0, &clear_color);

        // FIXME: Lots of duplication with composition rendering
        for (composition_data.objects.items, 0..) |item, comp_idx| {
            const compsoed_to_composition = item.composedToCompositionTransform(self.objects, composition_object_aspect);
            ...
            self.renderer.comp_id_program.render(
                ...
                &.{
                    ...
                    .{
                        ...
                        .val = .{ .mat3x3 = compsoed_to_composition.inner },
                    },
                },
            );
        }
    ...
    pub fn renderObjectWithTransform(self: *FrameRenderer, object: Object, transform: Transform) !void {
        switch (object.data) {
            .composition => |c| {
                ...
                if (c.debug_masks) {
                    const output_tex = try self.renderCompositionIdMask(object);
